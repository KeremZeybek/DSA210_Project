...
> *“Did governments use the power granted by COVID-19 restrictions as a political tool to repress their own citizens?”*
...
### Data from the ACLED (Armed Conflict Location & Event Data Project)
*Used to gather information about repressions, political violence and demonstrations.* [Link](https://acleddata.com)
...
* **Data on Repression (12-month lag):** * This is the primary dataset for the project. It contains data on state repression events worldwide from 2020 to 2024.
    * [Dataset Link](https://acleddata.com/themed/data-repression-12-month-lag)
...
* **Number of political violence events by country-month-year:** * This dataset provides monthly data on political violence to support the analysis.
    * [Dataset Link](https://acleddata.com/aggregated/number-political-violence-events-country-month-year)

* **Number of demonstration events by country-year:** * This file provides annual data on demonstration events.
    * [Dataset Link](https://acleddata.com/aggregated/number-demonstration-events-country-year)

### Additional Data Sources

* **OxCGRT (Oxford COVID-19 Government Response Tracker):** * This dataset provides the government responses and policies about the Covid-19 for every country from 2020 to June 2023.
    * [Project Link](https://www.bsg.ox.ac.uk/research/covid-19-government-response-tracker)
    * [Data Repo](https://github.com/OxCGRT/covid-policy-dataset/tree/main/data)
...
The raw OxCGRT dataset is too large for this analysis. Related data files will be selected. We have specifically selected the `OxCGRT_compact_national_v1.csv` file.

* **Granularity Match:** Our target variable (ACLED Repression Events) is aggregated at the National level. The `_national` version of the OxCGRT file comes pre-filtered to exclude sub-national regions (e.g., US states, Canadian provinces), significantly reducing file size and processing overhead.
...
* **Data Structure:** Unlike the timeseries files which use a wide format, the compact file uses a standard long format (one row per date), which is optimized for time-series resampling in Python/Pandas.
* **Column Selection:** To avoid memory issues, we will strictly load only essential columns (`CountryName`, `Date`, `EconomicIndex_Average`, and `StringencyIndex_Average`).
* **Temporal Downsampling:** The OxCGRT data is *daily*, while the ACLED aggregated data is *monthly*. We will apply **temporal downsampling** by grouping the daily `StringencyIndex_Average` entries by `Country` and `Month` and calculating the **mean**.
...
### Stage 1: EDA & Hypothesis Testing Findings (Completed)

Following the comprehensive data analysis of active conflict zones (2020-2023), we identified several critical patterns:

**1. Data Characteristics (EDA Results)**
...
**2. Temporal Dynamics (Lag Analysis)**
...
**3. Policy Efficacy Analysis ("Carrot & Stick")**
...
**4. Evolution of the Relationship (Rolling Correlation)**
...
1.  **The "Momentum Hypothesis":**
    * Current repression is the dominant predictor of future repression (coefficient = 0.70), confirming path dependency in state violence.

2.  **Linear Relationship:**
    * The superiority of Linear Regression over Random Forest proves that no "hidden complexity" exists—the mechanism is straightforward: *Policy Pressure + Lack of Support = Violence*.

3.  **The 16x Gap:**
    * Authoritarian regimes (India, Myanmar) experienced 116 repression events on average—**16 times higher** than Welfare States (7 events). This stark difference validates the "Carrot & Stick" hypothesis.

4.  **Economic Support as a Lever:**
    * 84 countries in the "Welfare State" cluster demonstrate that high restrictions paired with high economic aid leads to stability, not violence.

5.  **Outlier Analysis:**
    * India and Myanmar's extreme repression cannot be explained by policy alone—pre-existing conflicts, regime fragility, and population scale likely amplified pandemic pressures.
